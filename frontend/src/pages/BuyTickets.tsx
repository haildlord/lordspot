...
import { unpackTicketForUI, generateTicket } from "../utility/unpackForUi.ts";
...
export const BuyTickets = () => {
    ...
    // Add isAI boolean to track which tickets were generated by the Lord
    const [stagedTickets, setStagedTickets] = useState<{ normals: number[], bonus: number, isAI?: boolean }[]>([]);
    ...
    const handleGenerateRandom = () => {
        ...
        // Mark random tickets as isAI: false
        const newTickets = Array.from({ length: count }, () => ({ ...generateTicket(normal_marble_max, special_marble_max), isAI: false }));
        setStagedTickets(prev => [...prev, ...newTickets]);
        setGenCount(1);
    };

    const handleGenerateUsingAI = async () => {
        if (!connected || !publicKey || !wallet || isDrawing || genCount === 0 || !normal_marble_max || !special_marble_max) return;

        isCranking.current = true;
        setIsAILoading(true);

        try {
            // Fetch all tickets for the current epoch from Supabase
            const { data: ticketData, error: supabaseError } = await supabase
                .from('ticket_purchases')
                .select('packed_ticket')
                .eq('epoch_id', current_epoch_id);

            if (supabaseError) throw new Error(`Supabase Error: ${supabaseError.message}`);

            // Unpack the tickets into an array of arrays
            const previousTickets = (ticketData || []).map(row => {
                const unpacked = unpackTicketForUI(row.packed_ticket.toString(), normal_marble_max);
                return [...unpacked.numbers, unpacked.bonus];
            });

            // Send request to your AI Server with the past tickets included
            const response = await fetch(`${render_post_server_link}/api/ai-pick`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    totalPicks: genCount,
                    normalMax: normal_marble_max,
                    specialMax: special_marble_max,
                    pastTickets: previousTickets
                })
            });
            ...
            // Map the AI tickets and attach the isAI flag
            const newAITickets = data.tickets.map((t: any) => ({
                normals: t.normals,
                bonus: t.bonus,
                isAI: true
            }));

            // Stage the new AI tickets
            setStagedTickets(prev => [...prev, ...newAITickets]);
            setGenCount(1); // Reset input
    ...
    const handleAddManualTicket = () => {
        ...
        setStagedTickets(prev => [...prev, { normals: manualNormals, bonus: manualBonus, isAI: false }]);
        setManualNormals([]);
        setManualBonus(null);
    };
    ...
    return (
        <div className="relative min-h-[calc(100vh-64px)] md:min-h-[calc(100vh-80px)] pt-4 md:pt-6 pb-12 md:pb-16 px-4 sm:px-6 overflow-hidden bg-[#0a0a0a] text-white">
                ...
                {!connected ? (
                    ...
                    <div className="w-full bg-black/40 backdrop-blur-xl border border-[#D4AF37]/20 shadow-[0_4px_30px_rgba(0,0,0,0.5)] rounded-[1.25rem] md:rounded-[1.5rem] p-4 md:p-6 relative overflow-hidden flex flex-col h-[600px] md:h-[620px]">
                        ...
                        <div className="shrink-0 mb-4">
                            ...
                            {activeTab === 'quick' && (
                                <div className="flex gap-2 sm:gap-3 animate-fade-in h-9 md:h-10">
                                    <div className={`flex items-center justify-between bg-[#0d0d0d] border border-white/10 rounded-md md:rounded-lg p-1 w-24 sm:w-28 md:w-32 shrink-0 h-full ${maxCanStage === 0 ? 'opacity-50' : ''}`}>
                                        <button
                                            onClick={() => setGenCount(Math.max(1, (Number(genCount) || 0) - 1))}
                                            disabled={Number(genCount) <= 1 || maxCanStage === 0}
                                            className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 flex items-center justify-center text-lg md:text-xl text-slate-400 hover:text-[#D4AF37] transition-colors rounded hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed"
                                        >
                                            −
                                        </button>
                                        ...
                                        <input
                                            ...
                                            className="w-6 sm:w-8 md:w-10 text-center bg-transparent text-sm md:text-base font-black text-white focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none disabled:cursor-not-allowed"
                                        />
                                    ...
                                    <button onClick={handleGenerateRandom} disabled={maxCanStage === 0 || isAILoading} className="flex-1 bg-[#D4AF37]/10 border border-[#D4AF37]/30 hover:bg-[#D4AF37]/20 text-[#D4AF37] text-[9px] md:text-[10px] font-black tracking-[0.15em] uppercase rounded-md md:rounded-lg transition-all disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center gap-1.5 h-full active:scale-95">
                                    ...
                                    <div className="relative group flex-1 h-full">
                                        <button
                                            ...
                                            disabled={maxCanStage === 0 || isAILoading}
                                            className="w-full bg-gradient-to-r from-[#0891b2] via-[#6d28d9] to-[#e11d48] hover:from-[#06b6d4] hover:via-[#7c3aed] hover:to-[#f43f5e] text-white text-[8px] sm:text-[9px] md:text-[10px] font-black tracking-[0.15em] uppercase rounded-md md:rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1.5 h-full active:scale-95 shadow-[0_0_15px_rgba(8,145,178,0.4)] hover:shadow-[0_0_25px_rgba(225,29,72,0.6)] relative overflow-hidden border border-[#D4AF37]/40"
                                        >
                                            {/* Subtle inner glow to mimic gold jewelry */}
                                            <div className="absolute inset-0 border border-[#FFD700]/20 rounded-md md:rounded-lg pointer-events-none" />

                                            {isAILoading ? (
                                                <>
                                                    <div className="w-3.5 h-3.5 border-2 border-[#D4AF37]/30 border-t-[#FFD700] rounded-full animate-spin" />
                                                    <span className="text-[#FFD700] drop-shadow-[0_0_5px_rgba(255,215,0,0.5)]">LORD COMMUNICATES...</span>
                                                </>
                                            ) : (
                                                <>
                                                    {/* Mystical Eye Icon */}
                                                    <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#FFD700] drop-shadow-[0_0_5px_rgba(255,215,0,0.8)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                    </svg>
                                                    <span className="text-[#FFD700] drop-shadow-[0_0_5px_rgba(255,215,0,0.5)]">INVOKE LORD'S AI</span>
                                                </>
                                            )}
                                        </button>
                                        {/* Hover Tooltip */}
                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-64 p-3 bg-[#050505] border border-[#0891b2]/50 shadow-[0_0_25px_rgba(8,145,178,0.3)] rounded-xl text-[9px] text-slate-300 text-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-50">
                                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#06b6d4] to-[#f43f5e] font-black block mb-1.5 text-[10px] tracking-widest drop-shadow-md">DIVINE ORACLE AI</span>
                                            The Lord peers through the veils of human bias. By analyzing past epoch data, He generates sacred combinations that avoid crowded paths, maximizing your unique jackpot glory.
                                            <div className="absolute top-full left-1/2 -translate-x-1/2 border-[6px] border-transparent border-t-[#0891b2]/50" />
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            {stagedTickets.length === 0 ? (
                                ...
                                <div className="flex-1 overflow-y-auto pr-1 space-y-1.5 md:space-y-2 scrollbar-thin scrollbar-thumb-[#D4AF37]/30 scrollbar-track-transparent">
                                    {stagedTickets.map((ticket, i) => (
                                        <div key={i} className={`flex justify-between items-center bg-[#0a0a0a] border rounded-md md:rounded-lg p-1.5 md:p-2 transition-colors group relative overflow-hidden ${ticket.isAI ? 'border-[#0891b2]/50 hover:border-[#e11d48]/70 shadow-[0_0_12px_rgba(8,145,178,0.15)]' : 'border-white/5 hover:border-[#D4AF37]/30'}`}>

                                            {/* Subtle AI Glow Background */}
                                            {ticket.isAI && <div className="absolute inset-0 bg-gradient-to-r from-[#0891b2]/10 via-[#6d28d9]/10 to-[#e11d48]/10 pointer-events-none" />}

                                            <div className="flex items-center relative z-10">
                                                <span className={`font-black text-[8px] md:text-[9px] w-4 mr-1 md:mr-2 text-center ${ticket.isAI ? 'text-[#06b6d4] drop-shadow-[0_0_3px_rgba(6,182,212,0.8)]' : 'text-[#D4AF37]/50'}`}>
                                                    {ticket.isAI ? '🪶' : (i + 1)}
                                                </span>
                                                <div className="flex gap-1 md:gap-1.5">
                                                    {ticket.normals.map((num, idx) => (
                                                        <div key={idx} className={`w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center text-[8px] md:text-[9px] font-bold shadow-inner ${ticket.isAI ? 'bg-[#0f172a] border border-[#0891b2]/40 text-[#22d3ee] shadow-[inset_0_0_6px_rgba(8,145,178,0.3)]' : 'bg-[#111] border border-white/10 text-slate-300'}`}>
                                                            {num}
                                                        </div>
                                                    ))}
                                                    <div className={`w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center text-[8px] md:text-[9px] font-black ml-0.5 md:ml-1 ${ticket.isAI ? 'bg-gradient-to-br from-[#e11d48] to-[#f59e0b] text-white shadow-[0_0_6px_rgba(225,29,72,0.6)] ring-[1px] ring-[#FFD700]/60' : 'bg-gradient-to-br from-[#FFD700] to-[#B8860B] text-black shadow-[0_0_4px_rgba(212,175,55,0.3)] ring-[0.5px] ring-[#FFD700]/50'}`}>
                                                        {ticket.bonus}
                                                    </div>
                                                </div>
                                            </div>
                                            <button onClick={() => handleRemoveTicket(i)} className="relative z-10 w-5 h-5 md:w-6 md:h-6 rounded-full bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white flex items-center justify-center transition-colors opacity-100 lg:opacity-0 group-hover:opacity-100">
            ...
            {/* How to play / Prize Tier Overlays remain completely unchanged */}
            {isHowToPlayOpen && (
